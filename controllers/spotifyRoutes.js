...
router.get("/album/:album", checkIfLoggedInReroute, async (req, res) => {
  const accessToken = await getAccessToken();
  try {
    const specificAlbum = await fetch_album_data(accessToken, req.params.album);
...
router.get("/track/:track", checkIfLoggedInReroute, async (req, res) => {
  ...
  try {
    ...
    const specificTrack = await fetch_track_data(accessToken, req.params.track);
    ...
    if (
      specificTrack.trackName != "Slide" &&
      specificTrack.trackName != "BabyDrill"
    ) {
